# Robot Process: Tilsyn Image Forwarder

This process automates the transfer of documentation photos from Azure Blob Storage to the Vejman management system.

## Workflow
1. **Download**: Retrieves the image from Azure Blob Storage into memory using a connection string from Orchestrator.
2. **Upload**: Forwards the image to the Vejman API (`/permissions/file`) attached to the specific Case ID.
3. **Cleanup**: Performs a **hard delete** of the blob from Azure Storage upon successful confirmation from Vejman to minimize storage costs.

## Required Credentials
The following credentials must be configured in OpenOrchestrator:

| Credential Name | Field | Description |
| :--- | :--- | :--- |
| `TilsynBlobConnection` | **Username** | The Azure Container name (e.g., `tilsyn-uploads`). |
| | **Password** | The full Azure Storage Connection String. |
| `VejmanToken` | **Password** | The API token used for Vejman authentication. |
